...
rf = RandomForestClassifier(cacheNodeIds=True, featuresCol='scaledFeatures',labelCol ='disease_type_cpv_idx', maxDepth = 5, =200,\
                           seed=seed)
...
s3 = boto3.client('s3')
modelPath = 'full_rf_test_model/data/_SUCCESS'
if check(s3, 'gdc-emr0', stdmodelPath) == False:
    print("saving Random Forest model...")
    bestModel.save('s3://gdc-emr0/full_rf_test_model')
else:
    print(modelPath+" already exists...")
    bestModel.save('s3://gdc-emr0/full_rf_test_model_2')
...
s3_resource.Object('gdc-emr0', 'rf_test_feature_impt.csv').put(Body=csv_buffer.getvalue())
